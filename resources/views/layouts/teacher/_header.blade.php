    ...
    <div class="navbar-container d-flex content">
        ...
        <ul class="nav navbar-nav align-items-center ml-auto">
            ...
            @php
                ...
                $teacherOrganizations = auth()->user()->teacherOrganizations()->get();
                $selectedBranch = session('selected_branch');
                $teacherBranches = auth()->user()->teacherBranches()
            ...
            @if($selectedOrganization)
                ...
                <li class="nav-item dropdown dropdown-organization mr-1">
                    ...
                    @if($teacherOrganizations->count() > 1)
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-organization">
                            @foreach($teacherOrganizations as $organization)
                                <a class="dropdown-item {{ $organization->id == $selectedOrganization->id ? 'active' : '' }}"
                                   href="{{ route('teacher.profile.switch_organization', $organization) }}">
                                    {{ $organization->name }}
                                </a>
                            @endforeach
                        </div>
                ...
                <li class="nav-item dropdown dropdown-branch mr-1">
                    ...
                    @if($teacherBranches->count() > 1)
                        ...
                        <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-branch">
                            ...
                            @foreach($teacherBranches as $branch)
                                <a class="dropdown-item {{ $branch->id == $selectedBranch->id ? 'active' : '' }}"
                                   href="{{ route('teacher.profile.switch_branch', $branch) }}">
                                    {{ $branch->name }}
                                </a>
                            @endforeach
                        </div>
            ...
            <li class="nav-item dropdown dropdown-language">
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-flag">
                    ...
                    @foreach(config('localization.supportedLocales') as $localeCode => $properties)
                        ...
                        <a class="dropdown-item" href="{{ route('teacher.profile.switch_language', ['locale' => $localeCode]) }}">
            ...
            <li class="nav-item d-none d-lg-block">
                <a class="nav-link nav-link-style"
                   data-toggle-dark-model-url="{{ route('teacher.profile.toggle_dark_mode') }}"
                >
            ...
            <li class="nav-item dropdown dropdown-user">
                <a class="nav-link dropdown-toggle dropdown-user-link" id="dropdown-user" href="javascript:void(0);" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <div class="user-nav d-sm-flex d-none">
                        ...
                        <span class="user-status">@lang('users.teacher')</span>
                    </div>
                ...
                <div class="dropdown-menu dropdown-menu-right" aria-labelledby="dropdown-user">
                    ...
                    <a class="dropdown-item" href="{{ route('teacher.profile.edit') }}" wire:navigate>
                    ...
                    @if(session('impersonated_by'))
                        ...
                        <a class="dropdown-item" href="{{ route('teacher.profile.leave_impersonate') }}">
                            <i class="mr-50" data-feather="corner-up-left"></i>
                            @lang('admins.return_to_original_admin')
                        </a>
