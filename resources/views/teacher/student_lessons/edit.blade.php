...
<form method="post" action="{{ route('teacher.student_lessons.update', $studentLesson->id) }}"
    ...
    <div class="student-lesson-modal-wrapper">
                    ...
                    @foreach($evaluationItems as $item)
                        <div class="col-md-3 mb-1">
                            ...
                            >
                                @if($pageNumber !== null)
                                    @php
                                        $badgePageNumber = ($levelToPage !== null && $pageNumber >= $levelToPage)
                                            ? $pageNumber
                                            : $pageNumber + 1;
                                    @endphp
                                    <span class="page-number-badge">{{ $badgePageNumber }}</span>
                                @endif
                                {{ $item->name }}
                            </button>
                        </div>
        ...
        <div id="pages-with-evaluation-items" class="form-group" @if($lessonEvaluationItems->isEmpty()) style="display: none;" @endif>
