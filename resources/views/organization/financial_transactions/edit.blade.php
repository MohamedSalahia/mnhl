                ...
                <div class="col-md-8 offset-md-2">
                    <div class="card">
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('organization.financial_transactions.update', $financialTransaction->id) }}" class="ajax-form">
                                @csrf
                                @method('put')
                                    ...
                                    <div class="d-flex gap-2">
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="type" id="type-income" value="income"
                                                {{ old('type', $financialTransaction->getRawOriginal('type')) === 'income' ? 'checked' : '' }}>
                                        ...
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="type" id="type-expense" value="expense"
                                                {{ old('type', $financialTransaction->getRawOriginal('type')) === 'expense' ? 'checked' : '' }}>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="date" name="date" class="form-control"
                                           value="{{ old('date', $financialTransaction->date?->format('Y-m-d')) }}" required>
                                </div>

                                {{-- Description --}}
                                <div class="form-group">
                                    <label>@lang('financial_transactions.description')</label>
                                    <textarea name="description" class="form-control" rows="3" placeholder="@lang('financial_transactions.description_placeholder')">{{ old('description', $financialTransaction->description) }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="number" name="amount" class="form-control"
                                           value="{{ old('amount', $financialTransaction->amount) }}" step="0.001" min="0.001" required>
                                </div>
                                    ...
                                    <select name="currency_id" class="form-control select2">
                                        ...
                                        @foreach ($currencies as $currency)
                                            <option value="{{ $currency->id }}"
                                                {{ old('currency_id', $financialTransaction->currency_id) == $currency->id ? 'selected' : '' }}>
                                                {{ $currency->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select name="fund_id" class="form-control select2">
                                        ...
                                        @foreach ($funds as $fund)
                                            <option value="{{ $fund->id }}"
                                                {{ old('fund_id', $financialTransaction->fund_id) == $fund->id ? 'selected' : '' }}>
                                                {{ $fund->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="form-group">
                                    <button type="submit" class="btn btn-primary">
                                        <i data-feather="save"></i> @lang('site.edit')
                                    </button>
