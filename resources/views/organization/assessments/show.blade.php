                        ...
                        <div class="breadcrumb-wrapper col-12">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item">
                                    <a href="{{ route('organization.home') }}">@lang('site.home')</a>
                                </li>
                                <li class="breadcrumb-item">
                                    @php
                                        $statusLabels = [
                                    ...
                                    @endphp
                                    <a href="{{ route('organization.assessments.index', ['status' => $assessment->status]) }}">{{ $breadcrumbLabel }}</a>
                                </li>
                                    ...
                                    <div class="d-flex align-items-center justify-content-between">
                                        ...
                                        @if(auth()->user()->hasRole(UserTypeEnum::ORGANIZATION_SUPER_ADMIN))
                                            <div class="d-flex align-items-center">
                                                @if(in_array($assessment->status, [AssessmentStatusEnum::PENDING, AssessmentStatusEnum::IN_PROGRESS, AssessmentStatusEnum::PARTIALLY_IN_PROGRESS]))
                                                    @php
                                                        ...
                                                        $buttonRoute = $assessment->status === AssessmentStatusEnum::PENDING
                                                            ? route('organization.assessments.start', $assessment->id)
                                                            : route('organization.assessments.resume', $assessment->id);
                                                    @endphp
                                                    <a href="#" class="btn btn-light ajax-modal mr-1"
                                                ...
                                                @endif
                                                <button type="button" class="btn btn-light ajax-modal ml-1"
                                                        data-url="{{ route('organization.assessments.examiner.edit', $assessment->id) }}"
                                                        data-modal-title="@lang('assessments.edit_examiner')"
                                                        data-modal-size-class="modal-md"
                                                        style="background: rgba(255, 255, 255, 0.2); border: 1px solid rgba(255, 255, 255, 0.3); color: white; font-weight: 600;">
                                                    <i data-feather="edit"></i> @lang('assessments.change_examiner')
                                                </button>
                                            </div>
                                        @endif
                                    </div>
