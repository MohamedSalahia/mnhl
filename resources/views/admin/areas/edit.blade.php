                                ...
                                <div class="form-group">
                                    ...
                                    <select name="country_id" id="country-id" class="form-control select2" required
                                        ...
                                        @foreach ($countries as $country)
                                            <option value="{{ $country->id }}" {{ $country->id == $area->country->id ? 'selected' : '' }}>{{ $country->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    @foreach ($activeLanguages as $activeLanguage)
                                        ...
                                        <div class="col-md-{{ $activeLanguages->count() == 2 ? '6' : '12' }}">
                                            <div class="form-group">
                                                <label>@lang('areas.name') (@lang('languages.' . $activeLanguage->code))<span class="text-danger">*</span></label>
                                                <input type="text" name="{{ $activeLanguage->code }}[name]" data-error-name="{{ $activeLanguage->code }}.name"
                                                       {{ $loop->first ? 'autofocus' : '' }}
                                                       class="form-control"
                                                       value="{{ old('name', $area->translate($activeLanguage->code)?->name) }}"
                                                       required
                                                >
                                            </div>
                                        </div>
