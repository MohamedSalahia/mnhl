                    ...
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <form method="post" action="{{ route('admin.countries.update', $country->id) }}" class="ajax-form">
                                    ...
                                    @foreach ($activeLanguages as $activeLanguage)
                                        ...
                                        <div class="col-md-{{ $activeLanguages->count() == 2 ? '6' : '12' }}">
                                            <div class="form-group">
                                                <label>@lang('countries.name') (@lang('languages.' . $activeLanguage->code))<span class="text-danger">*</span></label>
                                                <input type="text" name="{{ $activeLanguage->code }}[name]" data-error-name="{{ $activeLanguage->code }}.name"
                                                       {{ $loop->first ? 'autofocus' : '' }}
                                                       class="form-control"
                                                       value="{{ old('name', $country->translate($activeLanguage->code)?->name) }}"
                                                       required
                                                >
                                            </div>
                                        </div>
                                ...
                                {{--mobile_digits--}}
                                <div class="form-group">
                                    <label>@lang('countries.mobile_digits') <span class="text-danger">*</span></label>
                                    <input type="number" name="mobile_digits" class="form-control" value="{{ old('mobile_digits', $country->mobile_digits) }}" required>
                                </div>
