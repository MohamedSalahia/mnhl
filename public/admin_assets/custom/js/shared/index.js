...
window.initIntlTel = () => {
    ...
    $.each($('.intl-tel'), function () {
        ...
        // Wrapper: direction:ltr so input is left, picker is right regardless of page RTL
        ...
        // Search input: stopPropagation to keep menu open
        ...
        // Build structure: wrap input, add picker
        $input.before($wrap);
        $wrap.append($input, $picker);

        // Initialize default country; detect from existing value if starts with +
        ...
        if (existingVal && existingVal.startsWith('+')) {
            ...
            if (match) {
                const stripped = existingVal.slice(match.dialCode.length + 1);
                $input.val(stripped);
                setSelected(match);
            } else {
    ...
    $(document).on('submit', '.ajax-form', function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response['reload_ajax_data_wrapper']) {

                    let reloadUrl = $('.ajax-data.active').data('url');
                ...
                window.handleContentsEmptyState();
